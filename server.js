...
app.get("/bxpro/v1/allmatch",async(req,res)=>{
    try {
        const response = await axios.get('https://betfairapi.turnkeyxgaming.com/api/v3/front/4',{ headers: {
...
app.get("/bxpro/v1/session/:eid",async(req,res)=>{
    const eventid = req.params.eid
    try {
        const response = await axios.get(`https://betfairapi.turnkeyxgaming.com/api/GetSession/${eventid}`,{ headers: {
                'x-turnkeyxgaming-key': '68c56ccbed10db48a50adc82',
                },})
        res.json(response.data)
    } catch (error) {
